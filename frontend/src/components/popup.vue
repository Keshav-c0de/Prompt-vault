...
onMounted(async () => {
  ...
  try {
    ...
    // If the EC2 server is down, or the internet disconnects, we catch it here
    console.error("Vault Connection Error:", error)
    errorMessage.value = "Unable to connect to the Vault. Please try again later."
  } finally {
    // The spinner turns off NO MATTER WHAT happens above
